...
The timer is armed only while a timed iterator `next()` is outstanding and rearms on `pulse()` for transport activity that yields no value, so consumer think time between reads never counts as idle. Call `watchdog.next(iterator, { idle: false })` for prefill demands before the first content token; pulse rearms only a timed outstanding demand. The interval must be positive, finite, and no greater than `MAX_TIMER_DELAY_MS`; Node otherwise clamps it to one millisecond. Zero is not a public disable for the watchdog.
...
`idleWatchdog` keeps one stable fused signal and arms the timer only while a timed `next()` is outstanding; `{ idle: false }` occupies the watchdog without arming (prefill). Resolution disarms, later timed demand or `pulse()` rearms, disposal clears, and concurrent demand rejects. Only the transport observes the signal, so the provider's real read must listen to it — the DeepSeek and pi-ai adapters close their response body or SDK request on abort.
