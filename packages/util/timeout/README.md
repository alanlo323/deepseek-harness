...
| `idleWatchdog(upstream, timeoutMs, code)` | Keep one stable fused signal and arm only while a **timed** guarded async-iterator `next()` is outstanding. Pass `{ idle: false }` to occupy the watchdog without arming (prefill). Resolution disarms; later timed demand or `pulse()` activity rearms; disposal clears; concurrent demand rejects. `timeoutMs <= 0` is rejected. |
...
For a streamed transport, create one `idleWatchdog`, pass its stable `signal` into the transport, and call `watchdog.next(iterator)` for each provider read after tokens have started. Call `watchdog.next(iterator, { idle: false })` for prefill demands before the first content token. Call `watchdog.pulse()` when transport activity does not yield an iterator value; pulse rearms only a timed outstanding demand. The interval must be positive, finite, and no greater than `MAX_TIMER_DELAY_MS`; Node otherwise clamps it to one millisecond. Zero is not a public disable for the watchdog. It measures only timed outstanding demand, so no timer runs while downstream code renders, waits before asking for the next chunk, or waits through silent prefill. The primitive still only notifies, so the transport must observe the stable signal; the DeepSeek and pi-ai adapters prove that timeout closes their real response body or SDK request.
...
- **An idle watchdog is not a total deadline** — it rearms per timed outstanding iterator demand, deliberately excludes consumer think time, and does not arm during untimed prefill.
