...
- **CJK prefers PingFang TC, then Noto Sans TC** — `DshCjk` uses `local()` PingFang TC when installed, otherwise the OFL Noto Sans TC Traditional webfont from `@fontsource/noto-sans-tc`, then Microsoft JhengHei. PingFang binaries are not bundled. See [Noto Sans TC webfont](../../../.agents/notes/implemented/feature/2026-08-18-web-cjk-noto-sans-tc-webfont.md).
