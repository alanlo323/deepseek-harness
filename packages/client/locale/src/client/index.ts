...
import { en, zh, zhHant, type CommonKey } from '../locales/index.ts'
import {
  en as settingsEn, zh as settingsZh, zhHant as settingsZhHant, type SettingsLocaleKey,
 ...
 * dictionaries carry identical key sets, so neither direction can
...
const BUILT_IN_LOCALE_METADATA = {
  ...
  'zh-Hant': { label: '繁體中文', fallback: 'zh' },
  en: { label: 'English' },
...
export class LocaleRuntime {
   ...
   * compile error), and every shipped locale is required (complete
   * {@link BuiltInLocaleId} coverage enforced at registration). Duplicate
   * (ns, locale) throws (single occupant; a namespace's texts have one owner).
   * Registration bumps the revision so mounted outlets pick up late-arriving dictionaries.
 ...
 * matches a locale id exactly, then a shipped Chinese script/region mapping,
 * then the primary subtag for every non-Chinese language, so an exact
 * regional registration wins before a language-wide fallback. Chinese skips
 * primary-subtag matching so `zh` does not claim `zh-TW`.
...
function detectBrowserLocale(locales: readonly LocaleDefinition[]): LocaleId | undefined {
  ...
  for (const tag of [...(languages ?? []), navigator.language]) {
    ...
    const chinese = matchShippedChinese(tag, locales)
    if (chinese !== undefined) return chinese
    const primary = requested.split('-')[0]
    /* v8 ignore next -- built-in `zh` makes matchShippedChinese succeed for every zh* tag. */
    if (primary === 'zh') continue
...
/**
 * Map one Chinese BCP 47 tag onto a shipped locale. `hans` selects `zh`;
 * `hant` or region `tw`/`hk` selects `zh-Hant`; remaining `zh*` (including
 * bare `zh` and `zh-CN`) selects `zh`. Returns undefined when this catalog
 * has neither the mapped id nor a `zh` fallback.
 * @param tag - A `navigator.languages` / `navigator.language` entry.
 * @param locales - definitions currently available to the browser.
 * @returns the matching shipped Chinese locale, or undefined.
 */
function matchShippedChinese(tag: string, locales: readonly LocaleDefinition[]): LocaleId | undefined {
  const parts = tag.toLowerCase().split('-')
  if (parts[0] !== 'zh') return undefined
  const has = (id: LocaleId) => locales.some(locale => locale.id === id)
  let id: LocaleId
  if (parts.includes('hans')) id = 'zh'
  else if (parts.includes('hant') || parts.includes('tw') || parts.includes('hk')) id = 'zh-Hant'
  else id = 'zh'
  if (has(id)) return id
  /* v8 ignore next -- LocaleRuntime never unloads built-in `zh`. */
  return has('zh') ? 'zh' : undefined
}
...
export function apply(ctx: ClientContext): void {
  ...
  locale.register(COMMON_NS, { zh, en, 'zh-Hant': zhHant })
  locale.register(SETTINGS_NS, { zh: settingsZh, en: settingsEn, 'zh-Hant': settingsZhHant })
