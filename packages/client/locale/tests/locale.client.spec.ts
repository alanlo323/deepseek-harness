...
describe('LocaleRuntime', () => {
  ...
  it('registers an external locale for selection, translation, persistence, and reversible disposal', () => {
    ...
    dispose()
    expect(svc.getLocale().active).toBe('zh')
    expect(svc.getLocale().locales.map(locale => locale.id)).toEqual(['zh', 'zh-Hant', 'en'])
  ...
  it('rejects a fallback cycle exposed by re-registering an unloaded language', () => {
    ...
    expect(svc.getLocale().locales.map(locale => locale.id)).toEqual(['zh', 'zh-Hant', 'en', 'fr-CA'])
  })
  ...
  it('matches a non-Chinese language pack by primary subtag after exact match misses', () => {
    stubLanguages('fr-CA')
    const { svc } = make()
    expect(svc.getLocale().active).toBe('en')
    svc.addLanguage({ id: 'fr', label: 'Français', fallback: 'en' })
  ...
  it('serves English as both the opening locale and the dictionary fallback', () => {
    ...
    // misses. Safe to share only because the shipped dictionaries carry
