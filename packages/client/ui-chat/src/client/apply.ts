...
import type { BoundActions, ObservableSnapshot, SnapshotStore } from '@deepseek-ai/dsh-client-store'
...
import { en, NS, zh, zhHant } from './locale.ts'
...
export function apply(ctx: Context): void {
  ...
  ctx.effect(() => ctx.locale.register(NS, { zh, en, 'zh-Hant': zhHant }), 'ui-chat: dictionaries')
  ...
  const thinkPreview = new ThinkPreviewPreference(
    ctx.settingsScope.bind<ThinkPreviewSettings>({ namespace: CONVERSATION_SETTINGS_NAMESPACE }),
  )
  const nodeInject = chatNodeInject(thinkPreview.mode)
