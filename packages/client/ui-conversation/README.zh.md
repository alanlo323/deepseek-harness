...
Think 行默认保持折叠。当推理块是流式输出尾部时，折叠摘要显示当前空行段落的开头，不会随每个 delta 追到行内末端；运行中的扫光仍是活跃提示（[决策](../../../.agents/notes/implemented/feature/2026-08-18-web-thinking-prefix-preview.md)）。Host 持久化的 `ui-conversation.collapsedThinkPreview` 通用设置可以把摘要改回最新行的末端跟随（[决策](../../../.agents/notes/implemented/feature/2026-08-02-web-thinking-tail-scroll.md)）。展开该行会移除折叠摘要，让完整推理进入普通页面流；结算后恢复左对齐的稳定首行摘要。
