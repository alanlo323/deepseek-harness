...
- **Empty environment** — the worker gets `env: {}` and does not inherit host `execArgv` (tsx, test-runner loaders). When native TypeScript is off in the host process, the unbuilt worker's `execArgv` is `--experimental-strip-types` and `--disable-warning=ExperimentalWarning` so `src/worker.ts` still loads without experimental warnings becoming program logs. No ambient credentials.
...
Source mode loads erasable-only `src/worker.ts` through Node's native type stripping. Host `pnpm dsh` is `node --import tsx/esm`; empty worker `execArgv` drops that loader rather than inheriting it. When `process.features.typescript` is off, the worker enables `--experimental-strip-types` and `--disable-warning=ExperimentalWarning` so `.ts` still loads. Its transitive runtime closure contains only Node built-ins and relative source modules, so a fresh checkout never requires a sibling workspace package's unbuilt `lib/` export. The worker-local and session-owned JSON boundaries both flatten and rebuild validated values around the message port so application nesting never reaches structured clone. Built mode passes the sibling `lib/worker.cjs` as a filesystem path because pkg's VFS Worker hook expects CommonJS; the same path works under ordinary Node. The repository-wide requirement to exercise this published entry path belongs to the [testing policy](../../../docs/testing.md).
