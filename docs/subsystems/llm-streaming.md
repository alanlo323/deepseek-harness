...
- **Provider stalls are bounded at the transport after the first non-empty content delta.** Both shipping remote adapters expose positive finite `streamIdleTimeoutMs` with a five-minute default. The watchdog arms only while a **timed** iterator `next()` is outstanding (after a non-empty text, reasoning, or tool-call argument delta), uses one stable signal for the whole request, maps its own expiry to `TIMEOUT`, and keeps an earlier caller abort as `ABORTED`. Silent prefill, a content `block-start`, and a named tool-call frame with empty arguments are not idle-capped.
