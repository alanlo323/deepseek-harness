...
- **Provider stalls are bounded at the transport after the first content token.** Both shipping remote adapters expose positive finite `streamIdleTimeoutMs` with a five-minute default. The watchdog arms only while a **timed** iterator `next()` is outstanding (after a content-bearing chunk), uses one stable signal for the whole request, maps its own expiry to `TIMEOUT`, and keeps an earlier caller abort as `ABORTED`. Silent prefill before that first content token is not idle-capped.
