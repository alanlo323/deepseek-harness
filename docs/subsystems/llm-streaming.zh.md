...
- **提供方停顿在传输层、且在第一个内容 token 之后受到时限约束。** 两个已交付的远程适配器都暴露正数且有限的 `streamIdleTimeoutMs`，默认五分钟。watchdog 只在**已计时**的 iterator `next()` 尚未完成时启动（在出现带内容的分片之后），整个请求使用同一个稳定 signal，把自身到期映射为 `TIMEOUT`，并把更早发生的调用方中止保留为 `ABORTED`。该第一个内容 token 之前的静默 prefill 不受空闲上限约束。
