...
- **提供方停顿在传输层、且在第一个非空内容 delta 之后受到时限约束。** 两个已交付的远程适配器都暴露正数且有限的 `streamIdleTimeoutMs`，默认五分钟。watchdog 只在**已计时**的 iterator `next()` 尚未完成时启动（在出现非空的 text、reasoning 或 tool-call 参数 delta 之后），整个请求使用同一个稳定 signal，把自身到期映射为 `TIMEOUT`，并把更早发生的调用方中止保留为 `ABORTED`。静默 prefill、内容 `block-start`，以及只有 name、参数为空的工具调用帧不受空闲上限约束。
