...
折叠 Think 摘要的横向布局读写最多每三帧执行一次，并直接追上该时刻的最新位置；React 仍按累计快照正常提交，定稿时摘要留在最后一段并回到左端（[结算末段决策](../feature/2026-08-20-web-thinking-settled-last-paragraph.md)）。该局部视觉策略不会改变正文滚动和用户交互的即时性。
