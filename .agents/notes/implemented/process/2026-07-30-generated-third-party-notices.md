...
**Keep `OFL-1.1` off the permissive set and authorize each font package by identity.** Rejected: OFL-1.1 is OSI-approved and describes bundling with software; a Claude-style identity exception would misstate those terms and would have to be copied for every open face.
