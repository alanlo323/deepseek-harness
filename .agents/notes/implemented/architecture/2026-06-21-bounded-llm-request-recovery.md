...
Each adapter exposes a validated `streamIdleTimeoutMs` configuration field with the five-minute prior-art default cited above. The interval is capped at Node's maximum timer delay so it cannot be clamped to one millisecond. After the first content token, it covers each outstanding iterator `next()` from demand to adapter-recognized provider activity; time a consumer spends between `next()` calls is not provider idle time. Prefill before that token is not idle-capped. DeepSeek SSE comments count as transport activity for a **timed** outstanding demand but never become `StreamChunk` values or session-log events, and they do not start the interval. See [prefill idle watchdog](../feature/2026-08-20-llm-prefill-idle-watchdog.md).
