...
Each adapter exposes a validated `streamIdleTimeoutMs` configuration field with the five-minute prior-art default cited above. The interval is capped at Node's maximum timer delay so it cannot be clamped to one millisecond. After the first non-empty content delta, it covers each outstanding iterator `next()` from demand to adapter-recognized provider activity; time a consumer spends between `next()` calls is not provider idle time. Prefill, a content `block-start`, and a named tool-call frame with empty arguments are not idle-capped. DeepSeek SSE comments count as transport activity for a **timed** outstanding demand but never become `StreamChunk` values or session-log events, and they do not start the interval. See [prefill idle watchdog](../feature/2026-08-20-llm-prefill-idle-watchdog.md).

`@deepseek-ai/dsh-timeout` exposes a rearmable idle-watchdog primitive. One stable local `AbortController` is fused with the caller signal and passed to the transport for the whole adapter call; each **timed** outstanding `next()` arms the watchdog, resolution disarms it, and the next timed demand rearms it. Untimed `next({ idle: false })` occupies the watchdog without arming. Out-of-band transport activity calls `pulse()` to rearm a timed outstanding demand without yielding a value. Timeout aborts that stable controller with a capability-owned `TimeoutReason`, and `finally` clears the timer. The adapter classifies its watchdog as `TIMEOUT` and an earlier upstream abort as `ABORTED`. The existing one-shot `deadline()` is not presented as a sliding timer.
