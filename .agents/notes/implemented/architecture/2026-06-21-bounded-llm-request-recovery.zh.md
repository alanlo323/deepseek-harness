...
每个适配器都公开一个经过验证的 `streamIdleTimeoutMs` 配置字段，默认值采用上文引用的五分钟先例。该间隔不超过 Node 的最大定时器延迟，因此不会被钳制为 1 毫秒。第一个非空内容 delta 之后，它覆盖每个尚未完成的迭代器 `next()`：从消费方请求下一项开始，到适配器识别到提供方活动为止；消费方在两次 `next()` 调用之间花费的时间不属于提供方空闲时间。Prefill、内容 `block-start`，以及只有 name、参数为空的工具调用帧不受空闲上限约束。DeepSeek SSE（Server-Sent Events）注释计为**已计时**尚未完成需求的传输活动，但绝不会成为 `StreamChunk` 值或会话日志事件，也不会启动该间隔。见[prefill 空闲看门狗](../feature/2026-08-20-llm-prefill-idle-watchdog.zh.md)。

`@deepseek-ai/dsh-timeout` 公开一个可重新布防的空闲看门狗原语。一个稳定的局部 `AbortController` 会与调用方信号融合，并在整个适配器调用期间传给传输层；每个**已计时**尚未完成的 `next()` 都会布防看门狗，该调用完成时解除布防，下一次已计时请求数据时再重新布防。未计时的 `next({ idle: false })` 占用看门狗但不布防。带外传输活动会调用 `pulse()`，在不产生值的情况下为已计时尚未完成的需求重新布防。超时会使用能力自身拥有的 `TimeoutReason` 中止这个稳定控制器，`finally` 则会清除定时器。适配器将自身看门狗归类为 `TIMEOUT`，将更早发生的上游中止归类为 `ABORTED`。现有的一次性 `deadline()` 不会被描述为滑动计时器。
