...
/** Arm only while one timed iterator `next()` is outstanding; `{ idle: false }` occupies without arming. */
...
| Arm and rearm only around timed outstanding iterator demand, including out-of-band activity | `dsh-timeout` (`idleWatchdog`) |
...
- **LLM adapters** — `dsh-llm-deepseek` and `dsh-llm-pi-ai` wrap actual transport iteration with `idleWatchdog`. The five-minute configured interval covers only timed outstanding provider demand after the first content token, not time the downstream consumer spends between chunks and not silent prefill. The direct DeepSeek adapter also pulses that timed outstanding demand when its SSE parser observes a comment, without yielding the comment as a `StreamChunk` or writing it to the session log. The pi-ai SDK does not expose comment activity to its adapter, so that path can rearm only when the SDK yields. The stable signal reaches `fetch` or the SDK for the whole call, so timeout closes the underlying request and maps to `TIMEOUT`, while an earlier caller abort maps to `ABORTED`. See [prefill idle watchdog](../feature/2026-08-20-llm-prefill-idle-watchdog.md).
...
- Model streams now share one rearmable timer contract without turning a sliding idle interval into a total-call deadline or charging consumer think time or silent prefill. Adapters that can observe out-of-band transport activity may pulse a timed outstanding demand; suppressed activity remains invisible to the watchdog. The primitive still only notifies; adapter tests prove their transports observe its stable signal and terminate.
