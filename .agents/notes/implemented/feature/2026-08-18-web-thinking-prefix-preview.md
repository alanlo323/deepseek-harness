...
Settled rows always restore `firstLine` of the entire reasoning block and `scrollLeft = 0`, regardless of the preference. Expanded `thinkBody` and Trajectory Thinking are unchanged. No session, wire, durable event, or model-visible contract changes.
