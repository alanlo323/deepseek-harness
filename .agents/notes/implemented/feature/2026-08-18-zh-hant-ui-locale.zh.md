...
**浏览器客户端出货三个 LocaleId：`zh`、`en` 与 `zh-Hant`。** 语言行标签为简体中文／繁體中文／English。`FALLBACK_LOCALE` 仍是 `zh`。显式 Host `locale.preference` 仍会实时替换暂定值，且探测结果从不写回 settings；见[由浏览器派生的初始 locale](2026-07-31-browser-derived-initial-locale.md)。
...
**分批出货 `zh-Hant` 词典，缺键回落到简体。** 否决：查找链已经回落到 `zh`，用户选了繁体后仍混用简体，会破坏类型化 `Record<LocaleId, dict>` 注册所要强制的完备性。
