...
两种模式的本地延迟都按指数增长，从 `initialDelayMs` 增至 `maxDelayMs`。省略策略时的 `maxDelayMs` 为 10000 毫秒；该默认值由[十秒延迟上限决策](2026-08-20-llm-retry-max-delay-ten-seconds.zh.md)规定。`jitterRatio` 用 `[1 - jitterRatio, 1 + jitterRatio]` 区间内的均匀随机样本乘以每次目标值，再应用上限。提供方给出的正数 `Retry-After` 若未超过上限，则保持精确且不加抖动。若提供方延迟超过上限，normal 模式会委托后续处理；always 模式则改用配置的本地退避，以维持无限重试保证。
