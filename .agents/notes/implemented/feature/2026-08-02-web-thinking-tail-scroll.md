...
`packages/client/ui-conversation/tests/reasoning-row.client.spec.tsx` pins the latest-line selection, the calculated right-edge scroll position, and the settlement reset to the first line and `scrollLeft = 0` when `previewMode` is `'follow-end'`. The default prefix path, live store toggle, and running-row `data-follow-end` absence are owned by the [prefix-preview decision](2026-08-18-web-thinking-prefix-preview.md). The keyless assembled Chromium scenario in `apps/web/tests/lifecycle-chrome.e2e.ts` keeps the settled replay golden unchanged, proving the historical first-line summary stays stable.
