...
`packages/client/ui-conversation/tests/reasoning-row.client.spec.tsx` pins the latest-line selection, the calculated right-edge scroll position, and settlement to the last paragraph with `scrollLeft = 0` when `previewMode` is `'follow-end'`. The default prefix path, live store toggle, and running-row `data-follow-end` absence are owned by the [prefix-preview decision](2026-08-18-web-thinking-prefix-preview.md). Settled replay Think names follow the last blank-line paragraph ([settled last-paragraph decision](2026-08-20-web-thinking-settled-last-paragraph.md)).
