...
2. **每次 run spawn 一个全新 `Worker`**，来自包自身的 bootstrap 模块：`env: {}`，`resourceLimits` 来自配置，`stdout`/`stderr` 捕获到 `logs` 而非继承。不继承宿主 loader 标志。未构建源码通过 Node 类型剥离加载 `src/worker.ts`。宿主 `pnpm dsh` 是 `node --import tsx/esm`；当 `process.features.typescript` 关闭时，worker 的 `execArgv` 为 `--experimental-strip-types` 和 `--disable-warning=ExperimentalWarning`，以便仍能加载该文件（[未构建 worker 的 strip `execArgv`](../bug-fix/2026-08-18-code-runtime-source-worker-strip-execargv.zh.md)）。构建模式加载兄弟文件 `lib/worker.cjs`。不做池化，不跨 run 保留状态：程序的世界随 worker 消亡，这使得 run 仅从日志即可重建，状态泄漏不可表达。
...
- **Worker 运行时：** 真实 worker 测试覆盖类型化的绑定值与失败、每一种无损 JSON 根类型的完成值、无效和超限输出、精确的组合账本边界、compute 和 wall 预算、恶意绑定流量、空环境以及 dispose 至完全停稳。source-worker 兼容测试会在 `node --import tsx/esm` 下启动未构建入口，并以隔离副本加上 `sourceWorkerExecArgv` 启动。一个构建后包测试在纯 Node 下运行 worker 入口。
