...
**Browser matching walks the ordered list; English matches the primary subtag, Chinese matches script then region.** `detectBrowserLocale()` walks `[...(navigator.languages ?? []), navigator.language]` and returns the first shipped locale that tag maps onto, so `en-GB` lands on `en`, `zh-Hans-CN` and `zh-Hans-TW` land on `zh`, and `zh-TW` / `zh-HK` / `zh-Hant*` land on `zh-Hant`, while a browser asking only for languages this app does not ship (`fr`, `de`) yields nothing and leaves `FALLBACK_LOCALE` in charge. `navigator.language` trails the list and covers its absence on hosts that ship a Navigator without `languages` — the DOM lib types it as always present, so that tolerance carries a narrow lint exception, the same environment-boundary distrust the `localStorage` guards already express. The Chinese script/region rule and the third shipped locale `zh-Hant` are owned by the [zh-Hant UI locale decision](2026-08-18-zh-hant-ui-locale.md).
...
- **Full BCP 47 negotiation (`Intl.LocaleMatcher`-style lookup, region and script weighting)**: rejected as an untestable extra layer. English still matches on the primary subtag; Chinese uses the small script-then-region rule owned by the [zh-Hant UI locale decision](2026-08-18-zh-hant-ui-locale.md) rather than a general matcher.
...
- A first visit from an English browser lands in English, and the Language row lists each shipped locale by its self-described label, so the escape hatch is available in every shipped language.
