...
Adapters keep `idleArmed` false until `startsPostTokenIdle(chunk)` is true: `isTokenDelta` or a `block-start` whose `blockType` is `text`, `reasoning`, or `tool-call`. Empty deltas do not start the cap. After that chunk is yielded, later `next()` calls are timed at the existing `streamIdleTimeoutMs`. Both adapters check `timeoutOf` before yield. User or turn cancellation during untimed prefill remains `ABORTED`.
